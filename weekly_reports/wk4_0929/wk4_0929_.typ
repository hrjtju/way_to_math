...
&= sum_(fp in X) sum_(k=1)^(K) f_k (fp) psi^((i))_k (fg^(-1) fu fp) #h(5em) & fp <- fu^(-1)fh \
&= sum_(fp in X) sum_(k=1)^(K) f_k (fp) psi^((i))_k ((fu^(-1) fg)^(-1) fp) \
...
$
["MaxPool" f](bx) = max_(bold(u) in cal(N)(bx)) f(bold(u))
$
其中 $cal(N)(bx)$ 是 $bx$ 的一个邻域，实践中常常为以 $bx$ 为中心的一个正方形邻域，如九宫格。

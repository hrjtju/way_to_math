...
- https://github.com/wolny/pytorch-3dunet/blob/master/pytorch3dunet/unet3d/se.py
...
def create_conv(
    ...
    for i, char in enumerate(order):
        if char == "l":
            ...
            raise ValueError(
                f"Unsupported layer type '{char}'. MUST be one of ['b', 'g', 'l', 'e', 'c', 'd', 'D']"
            )
...
class SingleConv(nn.Sequential):
    ...
    def __init__(
        ...
        for name, module in create_conv(
            in_channels, out_channels, kernel_size, order, num_groups, padding, dropout_prob
        ):
...
def get_model(model_config):
    model_class = eval(model_config["name"])
    print(model_class)
...
if __name__ == "__main__":
    ...
    arr = torch.randn((1, 1, 128, 128, 128))

...
> **定义 2.55 （历史）**
> $X(\cdot)$ 是一个实值随机过程，$\mathcal{U}(t):= \mathcal{U}\big( X(s): 0 \leqslant s \leqslant t \big)$ 是时刻 $0 \leqslant s \leqslant t$ 对应的随机变量 $X(s)$ 生成的 $\sigma$-代数。它叫做该过程直到时间 $t\geqslant {0}$ 的历史。

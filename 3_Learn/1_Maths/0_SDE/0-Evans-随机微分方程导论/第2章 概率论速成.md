...
> **定义 2.9 （随机变量，$\mathcal{U}$-可测）** 设 $(\Omega, \mathcal{U}, P)$ 是一个概率空间，一个映射 $$\boldsymbol{X}: \Omega \rightarrow \mathbb{R}^{n}$$被称为一个 $n$ 维随机变量，如果对任意 $B \in \mathcal{B}$，我们有 $$\boldsymbol{X}^{-1}(B) \in \mathcal{U}.$$我们也称 $\boldsymbol{X}$ 是 $\mathcal{U}$-可测的。（这说的其实是 $\boldsymbol{X}$ 是 $\mathcal{U}$ 到 $\mathcal{B}$ 的一个**可测映射**）

我们注意这里的 $X^{-1}(B) \in \mathcal{U}$，这样定义是比使用像的版本 $X(A) \in \mathcal{B}$ 要更好，需要记住的是，$\mathcal{U}$ 和 $\mathcal{B}$ 都是 $\sigma$-代数，所以必须满足定义 2.2 中所述的若干条件。考虑 $B_{1}, B_{2} \in \mathcal{B}$，我们理应有
$$
\begin{align}
...
> 显然，$\mathcal{U}(\boldsymbol{X})$ 要比原来的 $\mathcal{U}$ 小，而且它是使得 $\boldsymbol{X}$ 可测的最小的 $\mathcal{U}$ 的子$\sigma$-代数。
...
$$
![](Pasted%20image%2020250810175841.png)
...
> **定理 2.43 （强大数定律）**
> 考虑同一概率空间下一个独立同分布且可积的随机变量序列 $\boldsymbol{X}_{1}, \dots, \boldsymbol{X}_{n}, \dots$ ，记 $\boldsymbol{m} = E(X_{i}), i=1, \dots$，则有 $$P\left( \lim_{ n \to \infty } \frac{\boldsymbol{X}_{1} + \cdots + \boldsymbol{X}_{n}}{n} = \boldsymbol{m} \right) = 1. \quad 或者说 \quad \frac{1}{n}\sum\limits_{i=1}^{n} \boldsymbol{X}_{i} \rightarrow \boldsymbol{m}\quad \text{a.s.}$$
...
**证明.** 
如果 $\Phi$ 是线性的，结果显而易见
如果 $\Phi$ 不是线性的，构造 $S_{\Phi} := \{ (a, b): ax + b \leqslant \Phi(x), \forall x \in \mathbb{R} \}$，则 $\Phi(x) = \sup \{ax + b: (a, b) \in S_{\Phi}\}$。根据条件期望性质，有
$$
E(\Phi(X)|\mathcal{V}) \geqslant aE(X|\mathcal{V}) + b\quad \text{a.s.}
$$
对所有 $(a, b) \in S_{\Phi}$ 取上确界，就有
$$
E(\Phi(X)|\mathcal{V}) \geqslant \Phi(E(X|\mathcal{V}))\quad \text{a.s.}
$$
...
**赌徒谬误**
假设我们有一列相互独立的实值随机变量 $Y_{1}, Y_{2}, \dots$，它们都满足 $E(Y_{i}) = 0, i=1, 2, \dots$。我们可以定义它们的部分和 $S_{n}:= Y_{1} + \cdots + Y_{n}$。现在的问题是，给定了 $S_{1}, \dots, S_{n}$，我们想猜 $S_{n+k}$ 的值：
$$
\begin{align}
E(S_{n+k}|S_{1}, \dots, S_{n}) &= E(Y_{1} + \cdots + Y_{n}|S_{1}, \dots, S_{n}) + E(Y_{n+1} + \cdots + Y_{n+k}|S_{1}, \dots, S_{n})\\
&= Y_{1} + \cdots + Y_{n} + \underbrace{ E(Y_{n+1} + \cdots + Y_{n+k}) }_{ 0 }\\
&= S_{n}
\end{align}
$$
给定历史的部分和，可见 $S_{n+k}$ 的最好的估计就是 $S_{n}$。现在假设 $Y_{i}$ 是某个 ”公平“ 赌博游戏在时刻 $i$ 的结果，$S_{n}$ 就是 $n$ 时刻的总收益。所以上面的结果可以翻译为，给定目前的收益，将来任何时刻的期望收益和现在一样，也即上述情况描述了一个绝对公平的赌博游戏。我们可以将其形式化为鞅的定义

> **定义 2.54 （离散鞅）**
> $X_{1}, \dots, X_{n}, \dots$ 是一列满足 $E(|X_{i}|) < \infty, i=1, \dots$  的实值随机变量，如果它们满足$$X_{k} = E(X_{j}|X_{1}, \dots, X_{k})~\text{a.s.}\quad \forall j \geqslant k,$$我们称 $\{ X_{i} \}_{i=1}^{\infty}$ 是一个离散鞅。

> **定义 2.55 （历史）**
> $X(\cdot)$ 是一个实值随机过程，$\mathcal{U}(t):= \mathcal{U}\big( X(s): 0 \leqslant s \leqslant t \big)$ 是时刻 $0 \leqslant s \leqslant t$ 对应的随机变量 $X(s)$ 生成的 $\sigma$-代数。它叫做该过程直到8（并包括）时间 $t\geqslant {0}$ 的历史。

> **定义 2.56 （鞅）**
> $X(\cdot)$ 是一个随机过程，并满足对任意 $t \geqslant 0$，都有 $E(|X(t)|) < \infty$。
> （i）如果对所有 $t \geqslant s \geqslant 0$，都有 $X(s) = E(X(t)|\mathcal{U}(s))$，则 $X(\cdot)$ 是一个鞅
> （ii）如果对所有 $t \geqslant s \geqslant 0$，都有 $X(s) \leqslant E(X(t)|\mathcal{U}(s))$，则 $X(\cdot)$ 是一个**下**鞅

> **示例 2.57 （Brown 运动）**
> 一维 Brown 运动是一个鞅。

> **引理 2.58**
> $X(\cdot)$ 是一个实值鞅，$\Phi:\mathbb{R} \rightarrow \mathbb{R}$ 是凸函数。假设对任意 $t \geqslant 0$，都有 $E(|\Phi(X(t))|) < \infty$，则 $\Phi(X(\cdot))$ 是一个下鞅。

**证明.** 
如果 $\Phi$ 是线性的，有：
$$
\Phi(X(s)) = aX(s) + b = E(aX(t) + b|\mathcal{U}(s)) = E(\Phi(X)|\mathcal{U}(s))
$$
假如 $\Phi$ 不是线性的，那它被它的支持集 $S_{\Phi}= \{ (a, b): ax + b \leqslant \Phi(x) , \forall x \in \mathbb{R}\}$ 唯一确定：$$\Phi(x) = \sup\{ ax + b: (a, b) \in S_{\Phi} \}$$
由于对所有 $(a, b) \in S_{\Phi}$，都有 $\Phi(x) \geqslant ax + b$，就有
$$
E(\Phi(X(t))|\mathcal{U}(s)) \geqslant aE(X(t)|\mathcal{U}(s)) + b = aX(s) + b
$$
两边取上确界，有
$$
E(\Phi(X(t))|\mathcal{U}(s))\geqslant \Phi(X(s)).
$$
或者直接由条件 Jensen 不等式，立刻得到$$E(\Phi(X(t))|\mathcal{U}(s)) \geqslant \Phi(E(X(t)|\mathcal{U}(s))) = \Phi(X(t))$$因此 $\Phi(X(t))$ 是一个下鞅
...
> **定理 2.59 （离散鞅的若干不等式）**
> 1. $\{ X_{n} \}_{n=1}^{\infty}$ 是一个下鞅，则对所有 $n = 1, \dots$ 和 $\lambda > 0$，有 $$P\Big(\max_{1 \leqslant k \leqslant n} X_{k} \geqslant \lambda\Big) \leqslant \frac{1}{\lambda} E(X_{n}^{+})$$
> 2. $\{ X_{n} \}_{n=1}^{\infty}$ 是一个鞅，$1 < p < \infty$，则对所有 $n = 1, \dots$，有 $$E\Big( \max_{1 \leqslant k \leqslant n}|X_{k}|^{p} \Big) \leqslant \left( \frac{p}{p-1} \right)^{p}E(|X_{n}|^{p})$$

注意到上面的第一条定理给出了 Chebyshev 不等式的推广形式。下面是连续版本：

> **定理 2.59 （离散鞅的若干不等式）**
> 1. $X(\cdot)$ 是一个下鞅，则对所有 $t > 0$ 和 $\lambda > 0$，有 $$P\Big(\max_{0 \leqslant s \leqslant t} X(s) \geqslant \lambda\Big) \leqslant \frac{1}{\lambda} E(X(t)^{+})$$
> 2. $\{ X_{n} \}_{n=1}^{\infty}$ 是一个鞅，$1 < p < \infty$，有 $$E\Big( \max_{1 \leqslant s \leqslant t}|X(s)|^{p} \Big) \leqslant \left( \frac{p}{p-1} \right)^{p}E(|X(t)|^{p})$$

我们可以从离散版本的定理出发，证明连续的版本。选定 $\lambda > 0$，$t > 0$，然后做 $[0, t]$ 上的一个划分：$0 = t_{0} < t_{1} <\cdots <t_{n} = t$。这对应的离散鞅 $\{ X(t_{i}) \}_{i=1}^{n}$ 满足离散版本的不等式，然后让划分越来越细，最后取极限。
